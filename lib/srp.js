...
function padToNBuffer(number, params) {
...
Client.prototype = {
  ...
  setB: function setB(B_buf) {
    ...
    var B_bufPad = padTo(B_buf, 384); 
    var B_numPad = bignum.fromBuffer(B_bufPad);

    var u_num = getu(p.params, p.A_buf, B_bufPad);

    var S_buf = client_getS(p.params, p.k_num, p.x_num, p.a_num, B_numPad, u_num);

    p.K_buf = getK(p.params, S_buf);

    p.M1_buf = getM1(p.params, p.user, p.s_buf, p.A_buf, B_buf, p.K_buf);
  ...
  checkM2: function checkM2(serverM2) {
    ...
    if (p.M1_buf === undefined)
      throw new Error("incomplete protocol");
    if (!equal(getM2(p.params, p.A_buf, p.M1_buf, p.K_buf), serverM2))
      return false;
    return true;
  },
  computeK: function computeK() {
    var p = this._private;
    if (p.K_buf === undefined)
      throw new Error("incomplete protocol");
    return p.K_buf;
  }
};
